...
export default function BlogAdminPage() {
  ...
  const allTags = useMemo(() => {
    ...
    for (const post of posts) {
      if (post.tags) for (const tag of post.tags) tagSet.add(tag)
    }
  ...
  async function bulkDelete() {
    ...
    for (const id of selectedIds) await fetch(`/api/admin/blog/${id}`, { method: 'DELETE' })
  ...
  async function togglePublish(post: Post) {
    try {
      const res = await fetch(`/api/admin/blog/${post.id}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(
          post.published ? { published: false } : { published: true, published_at: new Date().toISOString() }
        ),
      })
      if (!res.ok) throw new Error('Failed to update')
      const updated = await res.json()
      setPosts(prev => prev.map(p => p.id === post.id ? { ...p, published: updated.published, published_at: updated.published_at } : p))
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const tags = exportTags.split(',').map(t => t.trim()).filter(Boolean)
    if (tags.length === 0) setExportCount(posts.length)
    else setExportCount(posts.filter(p => p.tags && tags.some(t => p.tags!.includes(t))).length)
    ...
    <div className="space-y-6">
      ...
      {allTags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 items-center">
          ...
          {activeTag && (
            <button onClick={() => setActiveTag(null)} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-0.5">
              <X className="h-3 w-3" /> Clear
            </button>
          )}
          {allTags.map(tag => (
            <Badge key={tag} variant={activeTag === tag ? 'default' : 'outline'} className="cursor-pointer text-xs" onClick={() => setActiveTag(activeTag === tag ? null : tag)}>
              {tag}
            </Badge>
          ))}
        </div>
      ...
      {filteredPosts.length > 0 && (
        <div className="flex items-center gap-3">
          ...
          {selectedIds.size > 0 && (
            <BulkActions posts={posts} selectedIds={selectedIds} onPublish={bulkPublish} onUnpublish={bulkUnpublish} onDelete={bulkDelete} />
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <p className="text-muted-foreground">{activeTag ? `No posts with tag "${activeTag}".` : 'No posts yet. Create one to get started.'}</p>
      ) : (
            ...
            <Card key={post.id}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 py-4">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <input type="checkbox" checked={selectedIds.has(post.id)} onChange={(e) => { const next = new Set(selectedIds); if (e.target.checked) next.add(post.id); else next.delete(post.id); setSelectedIds(next) }} className="rounded shrink-0" />
                  <div className="space-y-1 min-w-0 flex-1">
                    <CardTitle className="text-base flex items-center gap-2 flex-wrap">
                      {post.title}
                      <Badge variant={post.published ? 'default' : 'secondary'}>{post.published ? 'Published' : 'Draft'}</Badge>
                      {post.tags?.map(tag => (
                        <Badge key={tag} variant="outline" className="text-[10px] cursor-pointer" onClick={() => setActiveTag(tag)}>{tag}</Badge>
                      ))}
                    </CardTitle>
                    <CardDescription>
                ...
                <div className="flex gap-2 ml-4">
                  <Button variant="outline" size="sm" title={post.published ? 'Unpublish' : 'Publish'} onClick={() => togglePublish(post)}>
                    {post.published ? <XCircle className="h-3.5 w-3.5 text-muted-foreground" /> : <Check className="h-3.5 w-3.5 text-green-600" />}
                  </Button>
      ...
      <Dialog open={exportOpen} onOpenChange={setExportOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Download posts as a ZIP file.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>Filter by tags (optional, comma-separated)</Label>
              <Input value={exportTags} onChange={(e) => setExportTags(e.target.value)} placeholder="ai, education" />
            </div>
            <p className="text-sm text-muted-foreground">
              {loadingCount ? 'Counting…' : exportCount !== null ? `${exportCount} post${exportCount !== 1 ? 's' : ''} match${exportCount === 1 ? 'es' : ''}` : ''}
            </p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setExportOpen(false)}>Cancel</Button>
            <Button onClick={downloadExport} disabled={exportCount === 0}>Download ZIP</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
