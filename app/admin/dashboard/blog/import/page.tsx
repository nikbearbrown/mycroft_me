...
export default function BlogImportPage() {
  ...
  async function handleImport() {
    ...
    const allTags = tags.split(',').map(t => t.trim()).filter(Boolean)
    if (sourceLabel.trim()) {
    ...
    try {
      ...
      setResult(
        `Imported ${data.imported} posts, skipped ${data.skipped} (already exist). Total in ZIP: ${data.total}.`
      )
    } catch (err) {
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      >
        ...
        {file ? (
          ...
          <p className="text-muted-foreground">Drop a ZIP file here or click to select</p>
        )}
        <input
          ref={fileRef}
          type="file"
        ...
        <>
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="west-africa, ai, consulting"
            />
            <p className="text-xs text-muted-foreground">Applied to all imported posts</p>
          </div>
          <div className="space-y-2">
