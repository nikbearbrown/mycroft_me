...
export default function AnalysisBrowser({ groups }: { groups: Group[] }) {
  ...
  const allTags = useMemo(() => {
    ...
    groups.forEach(g =>
      g.docs.forEach(d => d.tags.forEach(t => counts.set(t, (counts.get(t) || 0) + 1)))
    )
    ...
    <>
      ...
      {totalDocs === 0 ? (
        <p className="text-muted-foreground">
          {query || activeTag ? 'No analyses match your search.' : 'No analyses yet.'}
        </p>
      ) : (
          ...
          {filteredGroups.map(g => (
            <section key={g.folder}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {g.docs.map(doc => (
                  <Link key={doc.slug} href={`/analysis/${doc.slug}`}>
